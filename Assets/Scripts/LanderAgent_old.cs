...
public class LanderAgentOld : RocketAgent
{
    ...
    void FixedUpdate()
    {
        ...
        {
            // consume fuel
            currentFuel = currentFuel - thrust * Time.deltaTime;
    ...
    {
        Vector3 targetDelta = transform.position - landingPad.transform.position;
        float logX = Mathf.Sign(targetDelta.x) * Mathf.Log(Mathf.Abs(targetDelta.x) + 1, 10);
        float logY = Mathf.Sign(targetDelta.y) * Mathf.Log(Mathf.Abs(targetDelta.y) + 1, 10);
        float logZ = Mathf.Sign(targetDelta.z) * Mathf.Log(Mathf.Abs(targetDelta.z) + 1, 10);
        ...
        Vector3 velocityVector = GetComponent<Rigidbody>().velocity * 0.1f;

        //Debug.Log("signed log distance: " + logTargetDelta);
        //Debug.Log("angular velocity: " + angularVelocity);
        //Debug.Log("regular velocity: " + velocityVector);
    ...
    public void CollisionDetected(Collision lc)
    {
        if (lc.gameObject.tag == "ground" || lc.gameObject.tag == "Finish")
        {
            ...
            // end episode 8 seconds after first touchdown
            if (touchDown == false)
            {
                touchDown = true;
                Invoke("EndEpisode", 8);
